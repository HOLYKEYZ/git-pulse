...
export default function ReactionPicker({ postId, onReact, currentReactions = [] }: ReactionPickerProps) {
  ...
  return (
    <button
      onClick={handleStar}
className={`flex items-center gap-1.5 transition-colors group ${isStarred ? 'text-git-star-active' : 'text-git-muted hover:text-git-star-active'}`}
      title="Star">
            ...
            <div className={`relative flex items-center justify-center ${isAnimating ? 'scale-125 transition-transform duration-200' : 'transition-transform duration-200'}`}>
                <svg aria-hidden="true" height="16" viewBox="0 0 16 16" width="16" className={`fill-current group-hover:bg-git-star-active/10 rounded pb-0.5 px-0.5 ${isStarred ? '' : 'opacity-80'}`}>
                    {isStarred ?
