...
export default async function NotificationsPage() {
    ...
    try {
        ...
        [dbNotifs, ghEvents] = await Promise.all([
            prisma.notification.findMany({
                where: { user: { username: session.user.login } },
                orderBy: { createdAt: "desc" },
                take: 50,
            }),
            token ? getGitHubTimelineEvents(session.user.login, token, 100) : Promise.resolve([])
        ]);
    ...
    const unified: UnifiedActivity[] = [
        ...
        ...ghEvents.map(parseGitHubEvent).filter(Boolean) as UnifiedActivity[]
